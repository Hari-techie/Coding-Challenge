...
Minimum possible answer: We will get the minimum answer when we give n books of the array to n students
(i.e. Each student will receive 1 book). Now, in this case, the maximum number of pages will be the maximum element in the array. 
...
The maximum no. of pages he/she will receive is the summation of array elements i.e. sum(arr[]). 
...
In order to calculate the number of students we will write a function, countStudents(). 
This function will take the array and ‘pages’ as parameters and return the number of students to whom we can allocate the books.

countStudents(arr[], pages):

We will first declare two variables i.e. ‘students’(stores the no. of students), and pagesStudent(stores the number of pages of a student). 
As we are starting with the first student, ‘students’ should be initialized with 1.
...
public class BookAllocation {
    ...
    public static int getNoOfStudent(int a[], int n, int maxPages) {
        int students = 1;
        int pages = 0;
        for(int i = 0; i < n; i++) {
    ...
    public static int findPages(int a[], int n, int m) {
        ...
        for(int i = 0; i < n; i ++) {
            high += a[i];
            low = Integer.max(low, a[i]);
        }

        while(low <= high) {
            mid = (low + high)/ 2;
            int students = getNoOfStudent(a, n, mid);
            if(students > m) {
                low = mid + 1;
            }
            else {
