...
public class RodCutting {
    ...
    public static int recFun(int ind ,int bag, int profit[]) {
        if(ind == 0) {
            return bag * profit[0];
        }

        int notTake = recFun(ind - 1,bag, profit);
        int take = 0;
        int rodLength = ind + 1;
        if(rodLength <= bag) {
            take = profit[ind] + recFun(ind, bag - rodLength, profit);
        }
    ...
    public static void main(String[] args) {
        ...
        int bag = 5;
        System.out.println(recFun(4, bag, profit));
    }
}
